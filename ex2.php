...
$inputArray = preg_split("/[\s,]+/", $inputString);

$goodCount = 0;

for ($i = 0; $i < count($inputArray); $i += 3) {

    $minMax = explode("-", $inputArray[$i]);

    $checkLetter = str_replace(":", "", $inputArray[$i + 1]);

    $password = str_split($inputArray[$i + 2]);
...
function ex1($inputArray, $goodCount)
{
    for ($i = 0; $i < count($inputArray); $i += 3) {
